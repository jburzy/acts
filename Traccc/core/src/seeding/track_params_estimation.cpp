...
/// Set the initial covariance of a bound track parameter object
void set_initial_covariance(bound_track_parameters<>& track_params,
                            const track_params_estimation_config& config) {
  for (std::size_t j = 0u; j < e_bound_size; ++j) {
    scalar var = config.initial_sigma.at(j) * config.initial_sigma.at(j);
    if (j == e_bound_qoverp) {
...
track_params_estimation::output_type track_params_estimation::operator()(
  ...
  const edm::seed_collection::const_device::size_type num_seeds = seeds.size();
  ...
  for (edm::seed_collection::const_device::size_type i = 0; i < num_seeds;
    ...
    seed_to_bound_param_vector(track_params, measurements, spacepoints,
                               seeds.at(i), bfield);
    set_initial_covariance(track_params, m_config);
  }
  ...
  host_detector_visitor<detector_type_list>(
      det, [&]<typename detector_traits_t>(
        ...
        for (edm::seed_collection::const_device::size_type i = 0;
             i < num_seeds; ++i) {
          bound_track_parameters<>& track_params = result.at(i);
          seed_to_bound_param_vector(track_params, detector, measurements,
                                     spacepoints, seeds.at(i), bfield);
          set_initial_covariance(track_params, m_config);
        }
      });
